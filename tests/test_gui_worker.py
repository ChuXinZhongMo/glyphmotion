...
def test_worker_success_puts_success_message() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="glyphmotion-test-") as out_dir:
        ...
        GlyphMotionApp._convert_worker(fake, request)
        message = fake.work_queue.get_nowait()
        assert isinstance(message, ConversionSuccess), f"got {message!r}"
        assert message.animation.frames
        assert message.preview
        assert all(path.exists() and path.stat().st_size > 0 for path in message.outputs)
...
def test_worker_failure_puts_failure_message() -> None:
    with tempfile.TemporaryDirectory(prefix="glyphmotion-test-") as out_dir:
        ...
        GlyphMotionApp._convert_worker(fake, request)
        message = fake.work_queue.get_nowait()
        assert isinstance(message, ConversionFailure), f"got {message!r}"
